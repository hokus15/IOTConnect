...
from iotconnect import __version__
...
def write_version(version):
    """Update IOTCOnnect constant file with new version."""
    with open("iotconnect/__version__.py") as fil:
        content = fil.read()
    ...
    with open("iotconnect/__version__.py", "wt") as fil:
        content = fil.write(content)

    json_content = {'version': f"{major}.{minor}.{patch}"}
    json_content.update({'major': f"{major}"})
    json_content.update({'minor': f"{minor}"})
    json_content.update({'patch': f"{patch}"})

    with open("version-stable.json", "wt") as fil:
        content = fil.write(json.dumps(json_content))


def main():
    """Execute script."""
    parser = argparse.ArgumentParser(description="Bump version of IOTConnect")
    parser.add_argument(
        "type",
